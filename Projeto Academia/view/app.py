...
# Janelas
    ...
    def Artes_marciais(self):
        ...
        label_image_treinos.grid(row=0, column=0, pady=0)
        ...
        btn_karate.grid(row=1, column=0, pady=20)

        # Frame inferior (usando CustomTkinter)
        frame_inferior = ctk.CTkFrame(background_frame, fg_color="#5ce1e6", corner_radius=0,height=30)
        frame_inferior.pack(side="bottom", fill="x", pady=10)

        btn_voltar = ctk.CTkButton(frame_inferior, text="Voltar", fg_color="#808080", hover_color="#A9A9A9", command=self.Home, font=("Arial", 18, "bold"), width=150, height=50)
    ...
    def Karate(self):
        ...
        label_numero = ctk.CTkLabel(central_frame, text="AAA", font=("Arial", 24))
        label_numero.grid(column=0,row=0,pady=0)

        # Cria um botão que gera e exibe o número diretamente ao ser clicado
        botao_gerar = ctk.CTkButton(central_frame, text="Gerar Número", command=lambda: label_numero.configure(text=str(random.randint(1, 100))))
        botao_gerar.grid(column=0,row=1,pady=20)

        btn_voltar = ctk.CTkButton(central_frame,text="voltar",fg_color="#808080", hover_color="#A9A9A9",command=self.Artes_marciais,font=("Arial", 18, "bold"), width=150, height=50)
        btn_voltar.grid(row=2, column=0)
